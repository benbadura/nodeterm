...
Desktop target/package coverage and remaining native launch limitations are tracked in
`docs/desktop-targets.md`. Build all runtime bundles through `npm run build`; Windows hooks and
canvas/context helpers use `native-helper-install.ts` + the packaged Node-mode Electron runtime.
Profile resolution belongs in core, applies only to cold local sessions, and must not change a
warm generation. Keep the NSIS live-process refusal: upgrading must never kill running agents.
...
(dispatches to the current OS's unsigned packaging script), `npm run dist:win` (unsigned x64
NSIS installer + zip, `--publish never`). `.github/workflows/release.yml` runs on
`release.published` (stable or prerelease), checks out the release tag and builds unsigned macOS
arm64+x64, Windows x64 and Linux x64 packages on GitHub-hosted runners. Each platform uploads its
installers and SHA-256 manifest to the existing release after packaged-runtime checks pass.
It preserves release notes/status, requires no signing secrets, and disables updater checks in
all shipped packages. No update metadata/blockmaps or duplicate Actions artifacts are uploaded.
Manual dispatch accepts an existing release tag for retries; the tag version must match
`package.json`. The legacy local `npm run release` remains a separate signed macOS build path.
See `docs/desktop-targets.md#github-releases` for the publishing procedure.

**Linux ships AppImage + deb + rpm**, all unsigned, built by the release workflow's Linux matrix
job on `ubuntu-24.04` (`npm run dist:linux` locally). Things about it that are easy to get wrong:
...
- **The rpm target shells out to the system `rpmbuild`.** electron-builder bundles fpm, but not
  rpmbuild, so the Linux job installs it explicitly (`apt-get install -y rpm`); without it the
...
- **When the updater is enabled, rpm uses manual updates**: `isManualUpdatePlatform`
...
`release.yml`'s Windows matrix job: after a GitHub release is published it uploads the NSIS
installer + zip as assets of that release. A Windows failure fails its matrix job without
cancelling the other platforms; there is no automatic promotion gate or update-feed leg:
